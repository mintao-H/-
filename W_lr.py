...
class LRWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.lr_testsize.setValue(0.01)
        ...
        for button_name in self.group_buttons:
            ...
            if button:
                self.groups[button] = []
                self.group_states[button] = {'buttons':[],'textBrowser':'','lbl_img':'','dataset':'','lr_testsize':0.1,'image_path':'','last_used_button': None} #初始化组别状态
        ...
        for button_name in self.page_buttons:
            ...
            if button:
                ...
                self.buttons.append(button)
                self.page_states[button] = {'buttons':[],'textBrowser':'','lbl_img':'','dataset':'','lr_testsize':0.1,'image_path':''}
    ...
    def train_lr(self):
        try:
            ...
            test_size = float(self.lr_testsize.value())
            ...
            if self.selected_group:
                ...
                self.page_states[self.selected_button]['lr_testsize'] = self.lr_testsize.value()
            #保存当前页面状态
    ...
    def add_button_to_selected_group(self,button):
        if self.selected_group:
            ...
            button.setStyleSheet("background-color:yellow;")
            self.page_states[button] = {'textBrowser':'','lbl_img':'','dataset':'','lr_testsize':0.1,'image_path':''}
            #保存新添加页面的状态
            #self.save_page_state(button)
            #自动选择新添加的页面
    ...
    def save_group_state(self, group_button):
        ...
        if group_button:
            ...
            state = {'buttons':[], 
                     ...
                     'lr_testsize':self.lr_testsize.value(),
    ...
    def restore_group_state(self, group):
        ...
        if group in self.group_states:
            ...
            self.lr_testsize.setValue(state['lr_testsize'])
