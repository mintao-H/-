...
from Ui_DecisionTree_Regression import Ui_MainWindow
...
class DecisionTree_Regression_Window(QMainWindow, Ui_MainWindow):
    def __init__(self):
        super(DecisionTree_Regression_Window, self).__init__()
        self.setupUi(self)
        pixmap = QPixmap(None)
        self.result_path = ''
        ...
        self.DSB_test_size.setValue(0.1)
        self.DSB_max_depth.setValue(3)
        ...
        for button_name in self.group_buttons:
            ...
            if button:
                self.groups[button] = []
                self.group_states[button] = {'buttons':[],'textBrowser':'','lbl_img':'','dataset':'','DSB_test_size':0.1,'DSB_max_depth':3,'image_path':'','last_used_button': None} #初始化组别状态
        ...
        for button_name in self.page_buttons:
            ...
            if button:
                ...
                self.buttons.append(button)
                self.page_states[button] = {'buttons':[],'textBrowser':'','lbl_img':'','dataset':'','DSB_test_size':0.01,'DSB_max_depth':3,'image_path':''}
    ...
    def train_lr(self):
        try:
            ...
            # 检查数据集的列数
            if data.shape[1] < 2:
                raise ValueError("数据集至少需要两列（特征列和标签列）")

            X = data.iloc[1:,:-1].values
            y = data.iloc[1:,-1].values

            test_size = float(self.DSB_test_size.value())
            X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, random_state=42)

            model = DecisionTreeRegressor(max_depth=int(self.DSB_max_depth.value()))
            model.fit(X_train, y_train)

            # 显示结果
            y_pred = model.predict(X_test)
            mse = mean_squared_error(y_test, y_pred)
            result_text = f"Dataset: {Dataset_name}\ntest_size: {test_size}\nmax_depth:{int(self.DSB_max_depth.value())}\nMean Squared Error: {mse:.4f}"
            ...
            tree.plot_tree(model, filled=True, feature_names=data.columns[:-1], class_names=True)
            #生成随机文件名
            ...
            if self.selected_group:
                ...
                self.page_states[self.selected_button]['DSB_test_size'] = self.DSB_test_size.value()
                self.page_states[self.selected_button]['DSB_max_depth'] = self.DSB_max_depth.value()
            #保存当前页面状态
    ...
    def add_button_to_selected_group(self,button):
        if self.selected_group:
            ...
            button.setStyleSheet("background-color:yellow;")
            self.page_states[button] = {'textBrowser':'','lbl_img':'','dataset':'','DSB_test_size':0.1,'DSB_max_depth':''}
            #保存新添加页面的状态
            #self.save_page_state(button)
            #自动选择新添加的页面
    ...
    def save_group_state(self, group_button):
        ...
        if group_button:
            ...
            state = {'buttons':[], 
                     ...
                     'DSB_test_size':self.DSB_test_size.value(),
                     'DSB_max_depth':self.DSB_max_depth.value(),
    ...
    def restore_group_state(self, group):
        ...
        if group in self.group_states:
            ...
            self.DSB_test_size.setValue(state['DSB_test_size'])
            self.DSB_max_depth.setValue(state['DSB_max_depth'])
...
if __name__ == '__main__':
    ...
    win = DecisionTree_Regression_Window()
    win.show()
    sys.exit(app.exec_())