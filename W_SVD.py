...
from Ui_SVD import Ui_MainWindow
...
class SVD_Window(QMainWindow, Ui_MainWindow):
    def __init__(self):
        super(SVD_Window, self).__init__()
        self.setupUi(self)
        pixmap = QPixmap(None)
        self.result_path = ''
        ...
        self.DSB_n_components.setValue(1)
        ...
        for button_name in self.group_buttons:
            ...
            if button:
                self.groups[button] = []
                self.group_states[button] = {'buttons':[],'textBrowser':'','lbl_img':'','dataset':'','DSB_n_components':0,'image_path':'','last_used_button': None} #初始化组别状态
        ...
        for button_name in self.page_buttons:
            ...
            if button:
                ...
                self.buttons.append(button)
                self.page_states[button] = {'buttons':[],'textBrowser':'','lbl_img':'','dataset':'','DSB_n_components':0,'image_path':''}
    ...
    def train_lr(self):
        try:
            ...
            n_components = int(self.DSB_n_components.value())

            svd = TruncatedSVD(n_components=n_components)
            X_reduced = svd.fit_transform(data)

            explained_variance_ratio = svd.explained_variance_ratio_
            self.textBrowser.setText(f"Dataset: {Dataset_name}\nn_components:{n_components}\nExplained Variance Ratio: {explained_variance_ratio}")
            if self.selected_group:
                self.group_states[self.selected_group]['textBrowser'] = explained_variance_ratio

            # 可视化SVD结果
            plt.figure(figsize=(10, 7))
            plt.scatter(X_reduced[:, 0], X_reduced[:, 1], alpha=0.5)
            plt.title(f'SVD降维结果 (n_components={n_components})')
            ...
            if self.selected_group:
                self.page_states[self.selected_button]['textBrowser'] = explained_variance_ratio
                ...
                self.page_states[self.selected_button]['DSB_n_components'] = self.DSB_n_components.value()
            #保存当前页面状态
    ...
    def add_button_to_selected_group(self,button):
        if self.selected_group:
            ...
            button.setStyleSheet("background-color:yellow;")
            self.page_states[button] = {'textBrowser':'','lbl_img':'','dataset':'','DSB_n_components':0.01,'image_path':''}
            #保存新添加页面的状态
            #self.save_page_state(button)
            #自动选择新添加的页面
    ...
    def save_group_state(self, group_button):
        ...
        if group_button:
            ...
            state = {'buttons':[], 
                     ...
                     'DSB_n_components':self.DSB_n_components.value(),
    ...
    def restore_group_state(self, group):
        ...
        if group in self.group_states:
            ...
            self.DSB_n_components.setValue(state['DSB_n_components'])
