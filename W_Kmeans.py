...
class Kmeans_Window(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        for button_name in self.group_buttons:
            ...
            if button:
                self.groups[button] = []
                self.group_states[button] = {'buttons':[],'textBrowser':'','lbl_img':'','dataset':'','n_clusters':2,'image_path':'','last_used_button': None} #初始化组别状态
        ...
        for button_name in self.page_buttons:
            ...
            if button:
                ...
                self.buttons.append(button)
                self.page_states[button] = {'buttons':[],'textBrowser':'','lbl_img':'','dataset':'','n_clusters':2,'image_path':''}
    ...
    def train_lr(self):
        try:
            ...
            if self.selected_group:
                self.group_states[self.selected_group]['textBrowser'] = result
            ...
            if self.selected_group:
                self.page_states[self.selected_button]['textBrowser'] = result
                ...
                self.page_states[self.selected_button]['n_clusters'] = self.n_clusters.value()
            #保存当前页面状态
    ...
    def restore_group_state(self, group):
        ...
        if group in self.group_states:
            ...
            self.n_clusters.setValue(state['n_clusters'])
