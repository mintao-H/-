...
from Ui_KNN_Classify import Ui_MainWindow
...
class KNN_Classify_Window(QMainWindow, Ui_MainWindow):
    def __init__(self):
        super(KNN_Classify_Window, self).__init__()
        self.setupUi(self)
        pixmap = QPixmap(None)
        self.result_path = ''
        ...
        self.KNN_Classify_testsize.setValue(0.1)
        ...
        for button_name in self.group_buttons:
            ...
            if button:
                self.groups[button] = []
                self.group_states[button] = {'buttons':[],'textBrowser':'','lbl_img':'','dataset':'','KNN_Classify_testsize':0.1,'n_neighbors':5,'image_path':'','last_used_button': None} #初始化组别状态
        ...
        for button_name in self.page_buttons:
            ...
            if button:
                ...
                self.buttons.append(button)
                self.page_states[button] = {'buttons':[],'textBrowser':'','lbl_img':'','dataset':'','KNN_Classify_testsize':0.1,'n_neighbors':5,'image_path':''}
    ...
    def train_lr(self):
        try:
            ...
            test_size = float(self.KNN_Classify_testsize.value())
            ...
            if self.selected_group:
                ...
                self.page_states[self.selected_button]['KNN_Classify_testsize'] = self.KNN_Classify_testsize.value()
    ...
    def save_group_state(self, group_button):
        ...
        if group_button:
            ...
            state = {'buttons':[], 
                     ...
                     'KNN_Classify_testsize':self.KNN_Classify_testsize.value(),
    ...
    def restore_group_state(self, group):
        ...
        if group in self.group_states:
            ...
            self.KNN_Classify_testsize.setValue(state['KNN_Classify_testsize'])
