...
from Ui_DBSCAN import Ui_MainWindow
...
class DBSCAN_Window(QMainWindow, Ui_MainWindow):
    def __init__(self):
        super(DBSCAN_Window, self).__init__()
        self.setupUi(self)
        pixmap = QPixmap(None)
        self.result_path = ''
        ...
        self.DSB_eps.setValue(0.01)
        self.DSB_min_samples.setValue(0)
        ...
        for button_name in self.group_buttons:
            ...
            if button:
                self.groups[button] = []
                self.group_states[button] = {'buttons':[],'textBrowser':'','lbl_img':'','dataset':'','DSB_eps':0.01,'DSB_min_samples':0,'image_path':'','last_used_button': None} #初始化组别状态
        ...
        for button_name in self.page_buttons:
            ...
            if button:
                ...
                self.buttons.append(button)
                self.page_states[button] = {'buttons':[],'textBrowser':'','lbl_img':'','dataset':'','DSB_eps':0.01,'DSB_min_samples':0,'image_path':''}
    ...
    def train_lr(self):
        try:
            ...
            # 获取eps和min_samples值
            eps = float(self.DSB_eps.value())
            min_samples = int(self.DSB_min_samples.value())
            ...
            ax.set_title('Estimated number of clusters: %d' % n_clusters)
            #生成随机文件名
            ...
            if self.selected_group:
                ...
                self.page_states[self.selected_button]['DSB_eps'] = self.DSB_eps.value()
                self.page_states[self.selected_button]['DSB_min_samples'] = self.DSB_min_samples.value()
            #保存当前页面状态
    ...
    def save_group_state(self, group_button):
        ...
        if group_button:
            ...
            state = {'buttons':[], 
                     ...
                     'DSB_eps':self.DSB_eps.value(),
                     'DSB_min_samples':self.DSB_min_samples.value(),
    ...
    def restore_group_state(self, group):
        ...
        if group in self.group_states:
            ...
            self.DSB_eps.setValue(state['DSB_eps'])
            self.DSB_min_samples.setValue(state['DSB_min_samples'])
